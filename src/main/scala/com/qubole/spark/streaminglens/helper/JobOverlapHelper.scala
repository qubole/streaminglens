...
import com.qubole.spark.streaminglens.common.MicroBatchContext
...
object JobOverlapHelper {
  ...
  /**
    ...
    * @param microBatchContext
    * @return
    */
  def makeJobLists(microBatchContext: MicroBatchContext): List[List[JobTimeSpan]] = {
    // lets first find the groups by ExecID
    val jobGroupListByExecID = microBatchContext.jobSQLExecIdMap.groupBy(x => x._2).map(x =>
      x._2.map(x => microBatchContext.jobMap(x._1)).toList).toList

    val allJobIDs = microBatchContext.jobMap.map(x => x._1).toSet
    ...
    for (elem <- leftJobIDs) {
      val jobTimeSpan = microBatchContext.jobMap(elem)
  ...
  def estimatedTimeSpentInJobs(microBatchContext: MicroBatchContext): Long = {
    val jobGroupsList = makeJobLists(microBatchContext)
    jobGroupsList.map(x => (x.map(x => x.endTime).max - x.map(x => x.startTime).min)).sum
  }

  def criticalPathForAllJobs(microBatchContext: MicroBatchContext): Long = {
    val jobGroupsList = makeJobLists(microBatchContext)
    jobGroupsList.map(group =>
      group.map(job => job.computeCriticalTimeForJob()).max).sum
  }
